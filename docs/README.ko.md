...
`docs/index.html`처럼 표시된 상대경로도 클릭할 수 있습니다. 터미널을 시작한 워크스페이스 폴더를 기준으로 파일을 찾고, 탐색기에서 해당 파일을 선택합니다. CLI가 경로를 여러 줄로 나누거나 들여쓰기와 줄 끝 공백을 넣어도 전체 경로를 인식하며, 그리드 너비를 바꾼 뒤에도 같은 파일을 가리킵니다.

### 0.7.1 개선 사항

웹 링크는 브라우저로, 폴더 경로는 탐색기로 열립니다. 파일 경로는 파일을 실행하지 않고 탐색기에서 해당 파일을 선택합니다. CLI가 글자 색상만 지정해 출력한 절대 경로와 URL도 인식하며, 줄바꿈된 경로와 따옴표로 감싼 공백 포함 경로를 지원합니다. 링크 텍스트를 드래그해 선택할 때는 열리지 않습니다.
